...
export default function BillingPage() {
  ...
  const { balanceQuery, transactionsQuery, depositMutation } = useBilling();
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <BrandCard header="Deposit Funds" style={{ marginBottom: 'var(--spacing-6)' }}>
          {depositMutation.isSuccess && (
            <div role="status" className="rounded-lg px-4 py-3 mb-4 text-sm border" style={{ backgroundColor: 'rgba(44,182,125,0.1)', borderColor: 'var(--color-success)', color: 'var(--color-success)' }}>
              Deposit successful!
            </div>
          )}
          {depositMutation.isError && (
            <div role="alert" className="rounded-lg px-4 py-3 mb-4 text-sm border" style={{ backgroundColor: 'rgba(255,77,77,0.1)', borderColor: 'var(--color-error)', color: 'var(--color-error)' }}>
                ...
                {transactionsQuery.data.map((tx) => (
                  <div key={tx.id} role="listitem" className="flex justify-between items-center px-4 py-3 rounded-lg" style={{ backgroundColor: 'var(--color-bg-elevated)' }}>
                    ...
                    <div className="text-right">
                      <p className="text-sm font-semibold" style={{ color: tx.type === 'withdrawal' ? 'var(--color-error)' : 'var(--color-success)' }}>
                        {tx.type === 'withdrawal' ? '-' : '+'}${tx.amount}
                      </p>
                      <p className="text-xs capitalize" style={{ color: tx.status === 'pending' ? 'var(--color-warning)' : 'var(--color-text-muted)' }}>
                        {tx.status}
                      </p>
                    </div>
                  </div>
